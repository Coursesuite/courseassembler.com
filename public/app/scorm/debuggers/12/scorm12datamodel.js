...
function SCORMapi1_2() {
        ...
        if (param == "") {
            if (Initialized) {
                ...
                //var callback = M.mod_scorm.connectPrereqCallback;
               	consoleLog("LMSFinish Callback", cmi, result, errorCode);
    ...
    function LMSSetValue (element,value) {
	    consoleLog("API LMS SET VALUE", element, value);
        errorCode = "0";
        if (Initialized) {
            if (element != "") {
                expression = new RegExp(CMIIndex,'g');
                            ...
                            if (errorCode == "0") {
                                if ((typeof eval('datamodel["'+elementmodel+'"].range')) != "undefined") {
                                    ...
                                    if ((value >= ranges[0]) && (value <= ranges[1])) {
                                        eval(element+'=value;');
                                        errorCode = "0";
                                       // consoleLog("lms set value", element, value);
                                                                                return "true";
                                    } else {
    ...
    function LMSCommit (param) {
        ...
        if (param == "") {
            if (Initialized) {
                ...
                // trigger TOC update
                var sURL = "http://moodle25" + "/mod/scorm/prereqs.php?a=5&scoid=12&attempt=1&mode=&currentorg=&sesskey=YoiI0DYj13";
                consoleLog("LMSCommit", sURL, result);
                //var callback = M.mod_scorm.connectPrereqCallback;
                //YUI().use('yui2-connection', function(Y) {
                //    Y.YUI2.util.Connect.asyncRequest('GET', sURL, callback, null);
                //});
    ...
    function StoreData(data,storetotaltime) {
        ...
        consoleLog("StoreData",datastring);
