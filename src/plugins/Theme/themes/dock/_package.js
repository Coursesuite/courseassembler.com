...
document.addEventListener("DOMContentLoaded", function domLoader(event) {
	...
	if (navigator.userAgent.toLowerCase().indexOf("mobile")!==-1) {
		...
		{{#is "open" theme.DOCK.start}}document.body.classList.add("active");
		{{else}}document.body.classList.remove("active");{{/is}}
	}
    ...
    function li(i,title,expandable,aud,att,child,depth) {
		...
		if (icons.completion) html.push('<span><span class="ca ca-unchecked checkmark" title="Ticked when completed"></span></span>');
		html.push('</div>');
		if (expandable == true) {
			html.push("<ol>");
		} else {
