...
document.addEventListener("DOMContentLoaded", function domLoader(event) {
    ...
    var menu = [], child = false, dest = document.getElementById('pages');
...
function load() {
	...
	if (current_page.hasOwnProperty('attachments') && current_page.attachments.length) {
		...
		if (!attache) {
			...
			document.querySelector('#pagefiles').appendChild(attache);
		}
		attache.innerHTML = '';
		current_page.attachments.forEach(function(value, index) {
			...
			a.insertAdjacentHTML('afterBegin', "<span class='ca ca-attachment' title='" + value.replace(/\.[^/.]+$/, "") + "'></span>&nbsp;");
